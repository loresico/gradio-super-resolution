    ...
    try:
        ...
        if not model_path.exists():
            if scale == 4:
                ...
                raise ValueError(
                    f"Scale {scale}x model not available. Only 4x model is used."
                )
        ...
        if body_keys:
            max_block = max(
                [int(k.split(".")[1]) for k in body_keys if k.split(".")[1].isdigit()]
            )
        ...
        model = RRDBNet(
            in_nc=in_channels, out_nc=3, nf=64, nb=num_blocks, gc=32, scale=scale
        )
    ...
    try:
        ...
        if scale_factor == 2:
            ...
            if model is None:
                return (
                    None,
                    "❌ **Error:** Failed to load 4x AI model for 2x upscaling. Check that model file exists in `model_dir/` folder.",
                )
        ...
        if model is None:
            return (
                None,
                "❌ **Error:** Failed to load AI model. Check that model file exists in `model_dir/` folder.",
            )
        ...
        print(
            f"🔍 Model expects {model_input_channels} channels, input has {img_tensor.shape[1]} channels"
        )
        ...
        if model_input_channels == 12 and img_tensor.shape[1] == 3:
            ...
            padding = torch.zeros(
                img_tensor.shape[0], 9, img_tensor.shape[2], img_tensor.shape[3]
            )
            img_tensor = torch.cat([img_tensor, padding], dim=1)
        elif model_input_channels != img_tensor.shape[1]:
            print(
                f"⚠️  Channel mismatch! Model expects {model_input_channels}, got {img_tensor.shape[1]}"
            )
        ...
        progress(
            0.35,
            desc=f"Enhancing {orig_width}×{orig_height} → {orig_width*scale_factor}×{orig_height*scale_factor}...",
        )
        with torch.no_grad():
        ...
        print(
            f"🔍 Output tensor: shape={output.shape}, min={output_min:.3f}, max={output_max:.3f}, mean={output_mean:.3f}"
        )
        ...
        for c in range(min(3, output.shape[0])):
            ...
            print(
                f"   Channel {c}: min={ch_min:.3f}, max={ch_max:.3f}, mean={ch_mean:.3f}"
            )
        ...
        print(
            f"🔍 Final output range: min={output.min().item():.3f}, max={output.max().item():.3f}, mean={output.mean().item():.3f}"
        )
        ...
        pixel_increase = (
            (new_width * new_height) / (orig_width * orig_height) - 1
        ) * 100
